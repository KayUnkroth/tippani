...
await check("rejects duplicate run and artifact identities", () => {
  ...
  assert.throws(() => validateRuns(broken, { config }), /distinct names, run IDs, artifact paths, and digests/);
});
...
await check("rejects N/A without a scenario-specific contract rationale", () => {
  ...
  assert.throws(() => validateRuns(broken, { config }), /scenario-specific contract rationale/);
});

await check("retains the correct named run position for every result", () => {
  validateRuns(runs, { config });
  const combined = combineResults(runs, expectedIds);
  const result = combined.find((item) => item.scenarioId === "S0-BCK-005");
  assert.deepEqual(Object.keys(result.evidence.runs), [
    "run-1",
    "run-2",
    "run-3",
  ]);
  assert.equal(result.evidence.runs["run-2"].status, "Pass");
});
...
console.log(`s0-aggregate-runs: ${pass} passed, ${fail} failed`);
